...
function createAvif({ fileBuffer, lossless }) {
  if (['jpeg', 'png', 'gif'].includes(fileType(fileBuffer))) {
    return sharp(fileBuffer).avif({
      quality: 50,
      lossless,
    }).toBuffer();
  }
...
function createWebp({ fileBuffer, fileExt, lossless }) {
  switch (fileExt) {
    ...
    default:
      if (['jpeg', 'png'].includes(fileType(fileBuffer))) {
        return sharp(fileBuffer).webp({
          quality: 85,
          lossless,
        }).toBuffer();
      }
