...
async function convert({ paths, lossless, avif, webp, force }) {
  ...
  log({
    title: `Converting ${totalPaths} ${getPlural(totalPaths, 'image', 'images')} (${lossless ? 'lossless' : 'lossy'})...`,
  });

  if (lossless) {
    log({
      title: 'Lossless conversion may take a long time',
    });
  }
...
function checkResult({ fileBuffer, filePath, outputFormat, force }) {
