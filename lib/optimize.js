...
async function optimize({ paths, lossless: isLossless }) {
  ...
  log({
    title: `Optimizing ${totalPaths} ${getPlural(totalPaths, 'image', 'images')} (${isLossless ? 'lossless' : 'lossy'})...`,
  });

  if (isLossless) {
    log({
      title: 'Lossless optimization may take a long time',
    });
  }
